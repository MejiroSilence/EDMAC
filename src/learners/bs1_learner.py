...
class BS1Learner:
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        for t in range(batch.max_seq_length):
            ...
            l_state.append(extra_losses["state"])
            l_q.append(extra_losses["q"])
        mac_out = th.stack(mac_out, dim=1)  # Concat over time
        l_state = th.stack(l_state, dim=1)[:, :-1] #b,t,1
        l_q = th.stack(l_q, dim=1)[:, :-1] #b,t,1
        ...
        masked_l_q = mask * l_q
        L_q = masked_l_q.sum() / mask_sum

        loss = L_td + self.args.loss_weight_state*L_state + self.args.loss_weight_q*L_q
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            self.logger.log_stat("loss_q", L_q.item(), t_env)
