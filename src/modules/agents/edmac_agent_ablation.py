...
class EDMACAblationAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(EDMACAblationAgent, self).__init__()
        self.args = args
        ...
        if args.enable_split:
            ...
            self.env_latent_decoder = nn.Sequential(
                nn.Linear(args.rnn_hidden_dim, args.rnn_hidden_dim),
                nn.LayerNorm(args.rnn_hidden_dim),
                self.activation_func,
                nn.Linear(args.rnn_hidden_dim, args.latent_dim)
            )
            self.dec_latent_decoder = nn.Sequential(
                nn.Linear(args.rnn_hidden_dim, args.rnn_hidden_dim),
                nn.LayerNorm(args.rnn_hidden_dim),
                self.activation_func,
                nn.Linear(args.rnn_hidden_dim, args.latent_dim)
        ...
        #dec msg aggregator
        dec_attn_head_num = args.attn_head_num * (1 if args.denoising else 2)
        if args.denoising:
            self.diff_attn = DiffAttention(args.rnn_hidden_dim, args.msg_dim, args.msg_dim, dec_attn_head_num, args.attn_embed_dim, args.diff_attn_lambda_init)
        else:
            self.diff_attn = CrossAttention(args.rnn_hidden_dim, args.msg_dim, args.msg_dim, dec_attn_head_num, args.attn_embed_dim)
        ...
        self.msgQ = nn.Sequential(
            nn.Linear(dec_attn_head_num*args.attn_embed_dim, args.rnn_hidden_dim),
            nn.LayerNorm(args.rnn_hidden_dim),
            self.activation_func,
            nn.Linear(args.rnn_hidden_dim, args.n_actions)
        )
    ...
    def forward(self, inputs, alpha=0, state=None, hidden_state=None, test_mode=False, calculate_loss=False):
        ...
        #msg generate
        if self.args.enable_split:
            w = self.seg_encoder(h)
            h_msg = self.full_infor_latent_encoder(h)
            h_env = h_msg * w
            h_dec = h_msg - h_env
        else:
            h_env =  self.env_latent_decoder(h)
            h_dec = self.dec_latent_decoder(h)
        ...
        #env msg aggregator
        h_back = h
        reconstructed_back = None
        true_state = None
        if self.args.enable_env:
        ...
        extra_losses = None
        if calculate_loss:
            extra_losses = self.get_loss(h_env, h_dec, reconstructed_back, true_state, h_back, b, a, Qfinal.view(b, a, -1), gaussian_dec)
    ...
    def get_loss(self, h_env, h_dec, reconstructed, state, h, b, a, q, g1): #TODO: size check
        extra_losses = {}
        extra_losses["enable_cos_loss"] = self.args.enable_env and self.args.enable_dec and self.args.loss_weight_cos>0
        extra_losses["enable_state_loss"] = self.args.enable_env and self.args.loss_weight_state>0
        extra_losses["enable_KL_loss"] = self.args.enable_dec and self.args.loss_weight_kl>0

        if extra_losses["enable_cos_loss"]:
        ...
        if extra_losses["enable_state_loss"]:
        ...
        if extra_losses["enable_KL_loss"]:
            #h: ba, -1
