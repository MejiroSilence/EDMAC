...
class RnnMsgAgent(nn.Module):
    ...
    def __init__(self, input_dim, args):
        ...
        self.fc_msg_1 = nn.Linear(input_dim, args.rnn_hidden_dim)
        self.fc_msg_2 = nn.Linear(args.rnn_hidden_dim, args.n_agents)
    ...
    def generate_send_prob(self, obs):
        ...
        f = self.fc_msg_1(obs)
        s = self.fc_msg_2(f)
        s = nn.Softmax(dim=-1)(s).view(batch, self.args.n_agents, s.shape[-1])
        return s
    ...
    def aggregate(self, query, key, value, hidden, send_target):
        ...
        _key = key.view(b, 1, -1).repeat(1, n_agents, 1)
        ...
        received_evidence = self.fast_combine_message(evidence,send_target)
    ...
    def combine_message(self, evidence, send_target):
        batch_evidence = evidence.detach()
        batch_send_target = send_target.transpose(-2, -1) #b,a,a
        alpha = batch_evidence + 1
        S = torch.sum(alpha, dim=-1, keepdim=True)
        batch_belief = batch_evidence / (S.expand(batch_evidence.shape))
        batch_uncertainty = self.args.n_actions / S
    ...
    #implement by EDMAC, after testing, i am sure this function's result is the same as combine_message
    @torch.no_grad()
    def fast_combine_message(self, evidence, send_target):
        batch_evidence = evidence.detach()
        batch_send_target = send_target.transpose(-2, -1).contiguous().unsqueeze(-1) #b,agent,agent,1 batch,reciever,sender,1
        alpha = batch_evidence + 1
        S = torch.sum(alpha, dim=-1, keepdim=True)
        batch_belief = batch_evidence / (S.expand_as(batch_evidence)) #b,sender,action
        batch_uncertainty = self.args.n_actions / S #b,sender,1

        b,agent,action = batch_belief.size()
        batch_belief = batch_belief.view(b,1,agent,action).expand(-1,agent,-1,-1) #b,receiver,sender,action
        batch_belief = batch_belief*batch_send_target.expand_as(batch_belief)
        batch_uncertainty = batch_uncertainty.view(b,1,agent,1).expand(-1,agent,-1,-1) #b,receiver,sender,1
        batch_uncertainty = torch.max(batch_uncertainty, (1-batch_send_target).expand_as(batch_uncertainty))
        
        b_plus_u = batch_belief + batch_uncertainty.expand_as(batch_belief) #b,receiver,sender,action
        b_plus_u = torch.prod(b_plus_u, dim=-2) #b,receiver,action
        final_u = torch.prod(batch_uncertainty, dim=-2) #b,receiver,1
        final_belief = b_plus_u - final_u.expand_as(b_plus_u)
        """
        I commented out the following lines of code for normalization because they don't affect the final result. 
        Whether or not normalization is performed, the results after the belief_to_evidence function are the same. 
        To make the computation faster, I commented out the normalization.
        """
        #C = final_belief.sum(-1, keepdim=True) + final_u
        #final_belief = final_belief/C.expand_as(final_belief)
        #final_u = final_u/C
        
        combined_evidence = self.belief_to_evidence(final_belief, final_u)
        combined_evidence = evidence + 0.1 * combined_evidence
        return combined_evidence
