...
class ActionMixer(nn.Module):
    def __init__(self, args, abs=True):
        super(ActionMixer, self).__init__()

        self.args = args
        self.n_agents = args.n_agents
        self.embed_dim = args.mixing_embed_dim
        self.input_dim = self.state_dim = int(np.prod(args.state_shape))# + args.n_actions * args.n_agents

        self.action_embed = nn.Linear(args.n_actions, self.embed_dim, bias=False)
    ...
    def forward(self, qvals, states, actions):
        ...
        actions = actions.reshape(b * t, self.args.n_agents, self.args.n_actions)
