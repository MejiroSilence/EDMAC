...
class EDMACClubAgent_head(nn.Module):
    def __init__(self, input_shape, args):
        super(EDMACClubAgent_head, self).__init__()
        self.args = args
        ...
        #dec msg aggregator
        self.diff_attn = CrossAttention(args.rnn_hidden_dim, args.msg_dim, args.msg_dim, args.attn_head_num * 2, args.attn_embed_dim)
        ...
        self.msgQ = nn.Sequential(
            nn.Linear(2*args.attn_head_num*args.attn_embed_dim, args.rnn_hidden_dim),
            self.activation_func,
            nn.Linear(args.rnn_hidden_dim, args.n_actions)
        )
        ...
        self.msg_estimator = nn.Sequential(
            nn.Linear(args.msg_dim, args.rnn_hidden_dim),
            self.activation_func,
            nn.Linear(args.rnn_hidden_dim, 2*args.msg_dim)
        )
    ...
    def get_loss(self, state1, state2, g1, g2, m1, m2):
        ...
        sigma = th.clamp(m2_para[:,:, self.args.msg_dim:], max=self.args.var_top, min=self.args.var_floor)
        g_m2 = D.Normal(m2_para[:,:,:self.args.msg_dim], th.exp(sigma * 0.5))
