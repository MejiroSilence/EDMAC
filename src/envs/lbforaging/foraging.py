...
class ForagingEnv(MultiAgentEnv):
    ...
    def __init__(self,
        ...
        env_id = "Foraging{4}-{0}x{0}-{1}p-{2}f{3}-v3".format(field_size, players, max_food,
    ...
    def step(self, actions):
        ...
        self.obs, rewards, dones, truncated, info = self.env.step(actions.cpu().numpy())

        self.agent_score += rewards

        reward = np.sum(rewards)
        terminated = dones or truncated#np.all(dones)
