...
class ObAttnAEEnc(nn.Module):
    ...
    def __init__(self,
        ...
        self.query = nn.Linear(input_shape, self.args.attn_embed_dim * attn_heads)
        self.key = nn.Linear(input_shape, self.args.attn_embed_dim * attn_heads)
        self.value = nn.Linear(input_shape, self.args.encoder_hidden_dim * attn_heads)

        if self.args.encoder_use_rnn:
            self.encoder_rnn = nn.GRUCell(self.args.encoder_hidden_dim * attn_heads, self.args.encoder_hidden_dim)
        else:
            self.encoder_rnn = nn.Linear(self.args.encoder_hidden_dim * attn_heads, self.args.encoder_hidden_dim)
    ...
    def encode(self, inputs, encoder_hidden_state):
        ...
        query = self.query(inputs).reshape(bs, self.n_agents, self.attn_heads, self.args.attn_embed_dim)
        key = self.key(inputs).reshape(bs, self.n_agents, self.attn_heads, self.args.attn_embed_dim)
        value = self.value(inputs).reshape(bs, self.n_agents, self.attn_heads, self.args.encoder_hidden_dim)
        key = key.transpose(1, 2).contiguous().view(-1, self.n_agents, self.args.attn_embed_dim)
        query = query.transpose(1, 2).contiguous().view(-1, self.n_agents, self.args.attn_embed_dim)
        value = value.transpose(1, 2).contiguous().view(-1, self.n_agents, self.args.encoder_hidden_dim)
        query = query / (self.args.attn_embed_dim ** (1/4))
        value = value / (self.args.attn_embed_dim ** (1/4))
        dot = torch.bmm(query, key.transpose(1, 2))
        dot = F.softmax(dot, dim=2)
        out = torch.bmm(dot, value).view(bs, self.attn_heads, self.n_agents, self.args.encoder_hidden_dim)
        attn_out = out.transpose(1, 2).contiguous().view(bs * self.n_agents, -1)
